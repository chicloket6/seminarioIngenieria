        ...
        @if (count($options))
            @foreach ($options as $option)
                @if($current_value == $option->getKey())
                    <option value="{{ $option->getKey() }}" selected>{{ $option->{$field['attribute']} . ($option->tipoHabitacion ? ' - ' . $option->tipoHabitacion->nombre : '') }}</option>
                @else
                    <option value="{{ $option->getKey() }}">{{ $option->{$field['attribute']} . ($option->tipoHabitacion ? ' - ' . $option->tipoHabitacion->nombre : '') }}</option>
                @endif
            @endforeach
        @endif
    </select>
